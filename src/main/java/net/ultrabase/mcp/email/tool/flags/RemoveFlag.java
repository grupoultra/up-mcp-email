...
/**
 * Removes a flag or keyword from one or more emails.
 *
 * @author César Obach
 */
public class RemoveFlag extends BaseTool {

    public RemoveFlag(ToolContext context) {
        super(context);
    }
    ...
    public String getDescription() {
        return "Remove a flag or keyword from one or more emails. " +
            "Use '\\\\Flagged' to unflag, or specify keywords to remove.";
    }

    @Override
    public String getInputSchema() {
        return schema(
            "email_ids", "array:string", "List of email_id to unflag (obtained from list_emails_metadata)",
            "flags", "array:string", "Flags to remove. Use '\\\\Flagged' to remove standard flag, or keywords like 'Personal', 'Alta'.",
    ...
    public CompletableFuture<?> execute(Map<String, Object> args) {
        ...
        return context.emailClient(accountName).removeFlags(mailbox, emailIds, flags);
    }
}
